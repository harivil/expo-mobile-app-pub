...
// Matches the app source, which create-expo-app writes with single quotes. Formatting is
// arbitrary; agreeing on it is not, and the only wrong answer is two answers in one repo.
const PRETTIERRC = `{
  "semi": true,
  "singleQuote": true,
  "trailingComma": "all",
...
// Husky v9+: the hook file is the command, with no shebang boilerplate.
...
// The Claude Code hooks bind an agent session in this directory. This one is a git hook, so it
// binds whoever pushes from this clone — which is the only free way to protect main on a
// private repo, since GitHub's branch protection needs a paid plan.
const PRE_PUSH = `node .claude/hooks/guard-push.mjs
`;
...
if (install.size) {
  ...
  console.log(`    npx expo install -- -D ${[...install].join(" ")}`);
