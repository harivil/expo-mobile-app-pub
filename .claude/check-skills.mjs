...
// ---------------------------------------------------------------- CI parity
//
// ci-local.mjs claims to run what CI runs. That claim decays the moment someone adds a job to a
// workflow and not to the script — silently, because the script still passes and now proves
// less than the person running it believes. The preflight-ci skill says to change both; this is
// what makes that hold when nobody remembers reading it.
//
// A new job forces a decision rather than a default: implement it locally, or name it here as
// something no local run can do.
{
  const LOCAL_EXEMPT = new Map([
    ["detect", "decides whether the app exists on this ref — has no local meaning"],
    ["codeql", "no practical local runner; needs GitHub Code Security on a private repo"],
    [
      "zap-web",
      "weekly and on paths that shape served output, not per-PR — see the dast-scan skill",
    ],
  ]);
  ...
  if (existsSync(script) && existsSync(workflows)) {
    ...
    for (const m of scriptText.matchAll(/^\s*job:\s*"([^"]+)"/gm)) {
      // A check can stand in for more than one job — expo-doctor runs in two.
